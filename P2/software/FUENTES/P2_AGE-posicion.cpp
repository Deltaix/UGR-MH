...
void reparar(bool* &cromosoma, vector<float> &contribucionSel, vector<float> &contribucionNoSel, int n, int m) {
    ...
    if (contador > m) {
        ...
        while (diferencia > 0) {
            ...
            cromosoma[indice] = 0;
            contribucionSel.erase(contribucionSel.begin()+indice);
            maxEvaluacion = 0.0;
            diferencia--;
        }
    }
    ...
    if (contador < m) {
        ...
        while (diferencia > 0) {
            ...
            cromosoma[indice] = 1;
            contribucionNoSel.erase(contribucionNoSel.begin()+indice);
            maxEvaluacion = 0.0;
            diferencia--; 
        }
    }
    ...
    while (ruta != "QUIT") {
        ...
        if (archivo.is_open()) {
            ...
            for (i = 0; i < 100000; i = i + 2) {
                ...
                cruce(hijo1, hijo2, padre1, padre2, n);
			...
			cout << "--- Valor: " << fixed << mejorEvaluacion << endl;
