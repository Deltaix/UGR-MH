...
int main() {
    ...
    while (ruta != "QUIT") {
        ...
        if (archivo.is_open()) {
            ...
            // Otros parámetros
            int max_vecinos, max_exitos, n_iter, n_sol, M;
            int vecinos, exitos;
            ...
            // Aplicamos enfriamiento 10 veces
            int n = 0;
            while (n_sol < 10) {
                ...
                while (temperaturaFinal < temperaturaActual && i < n_iter && !final) {
                    ...
                    while (vecinos < max_vecinos && exitos < max_exitos) {
                        ...
                        for (j = 0; j < m; j++) {
                        	if (j != indice1) {
                            	nuevoCoste = nuevoCoste + distancias[noSeleccionados[indice2]][seleccionados[j]] - distancias[seleccionados[indice1]][seleccionados[j]];
                            }
                        }
                        ...
                        if (diferencia < 0 || randombinary(rng) <= exp(-diferencia/temperaturaActual)) {
                            ...
                            if (coste > mejorCoste) {
                                ...
                                for (j = 0; j < (n-m); j++) {
                                	if (j < m)
                                		mejorSolucion[j] = seleccionados[j];
                                	mejorResto[j] = noSeleccionados[j];
								}
                            }
                            exitos++;
                        }
                ...
                for (i = 0; i < (n-m); i++ ) {
                	if (i < m)
                		seleccionados[i] = mejorSolucion[i];
                	noSeleccionados[i] = mejorResto[i];
				}
                coste = mejorCoste;
                int indice1, indice2, tmp;
                ...
                for (int cont = 0; cont < t; cont++) {
                    ...
                    for (j = 0; j < m; j++) {
						if (j != indice1) {
							coste = coste + distancias[noSeleccionados[indice2]][seleccionados[j]] - distancias[seleccionados[indice1]][seleccionados[j]];
						}
					}
